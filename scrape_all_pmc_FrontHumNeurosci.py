...
df_keep = df[df['Journal Title']=='Front Hum Neurosci']
...
for i, row in df_keep.loc[N_previous + 1:].iterrows():
    ...
    if (i % N_save == 0) & (i > 0):
        ...
        df_save.to_csv('data/ofactor_{:d}_Front_Hum_Neurosci.csv'.format(i))
        if (i > N_previous + N_save):
            os.remove('data/ofactor_{:d}_Front_Hum_Neurosci.csv'.format(i - N_save))
